...
// Defalt is an alias for factory Default logger
func Default() *Log {
	return DefaultLogFactory().Default()
}

// NewLogger creates new Log object
...
// Default returns one default Log
func (f *Factory) Default() *Log {
	if f.defaultLog == nil {
		f.defaultLog = f.NewLogger()
	}
	return f.defaultLog
}
...
// Event
func (l *Log) Event(format string, s ...any) {
	if l.Level > LogLevelInfo {
		return
	}
	stdLog.Printf(Cyan+format+Reset, s...)
}
