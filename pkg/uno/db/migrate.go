...
func Migrate(db *sql.DB) {
	var migrations = os.DirFS("db/migrations")
	log.Default().Debug("Migrating from Dir: %s", migrations)
	if err := Up(context.Background(), db, migrations); err != nil {
		panic(err)
	}
